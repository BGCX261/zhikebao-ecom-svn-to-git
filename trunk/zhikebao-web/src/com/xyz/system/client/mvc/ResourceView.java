...
public class ResourceView extends LayoutContainer {
	...
	protected void onRender(Element parent, int index) {
		...
		final ListStore<Resource> store = new ListStore<Resource>(loader);
		...
		final RowEditor<Resource> re = new RowEditor<Resource>();
		...
		re.addListener(Events.AfterEdit, new Listener<RowEditorEvent>() {
			public void handleEvent(RowEditorEvent be) {
				...
				final Resource res = (Resource)rec.getModel();
				rec.isEditing();
		...
		final Grid<Resource> grid = new Grid<Resource>(store, cm);
		// 添加翻页工具条
		grid.addListener(Events.Attach, new Listener<GridEvent<Resource>>() {
		...
		toolBar.add(new Button(msg.delete(), General.ICONS.delete(),
				new SelectionListener<ButtonEvent>() {
					@Override
					public void componentSelected(ButtonEvent ce) {
						List<Resource> ls = store.findModels("deleted", true);
						for (final Resource r : ls) {
							MessageBox.confirm(msg.notice(), msg.confirmMsg(),
									new Listener<MessageBoxEvent>() {
										public void handleEvent(MessageBoxEvent ce) {
                                            ...
                                            {
                                            	service.delResource(r.getKey(), new AsyncCallback<Boolean>() {
                        							public void onFailure(Throwable caught) {
                        								Dispatcher.forwardEvent(Events.OnError, caught);
                        							}
