...
public class OnsaleItemView extends LayoutContainer {
	...
	private BasePagingLoader<PagingLoadResult<Item>> loader;
	...
	public OnsaleItemView() {
		...
		statusRen = new GridCellRenderer<BeanModel>() {
			public String render(BeanModel model, String property,
				...
				String sta = "onsale".equals(val) ? "出售中" : "库中";
				return sta;
			}
		};
		...
		gridNumber = new GridCellRenderer<BeanModel>() {
			public String render(BeanModel model, String property,
				...
				String val = (String) model.get(property);
				if(val != null)
				   return numberRenderer.render(null, property, Double.parseDouble(val));
				else
					return "";
			}
		};
	...
	private void createExpander() {
		...
		loader = new BasePagingLoader<PagingLoadResult<Item>>(proxy,
				new BeanModelReader());

		final PagingToolBar pagingBar = new PagingToolBar(18);
		...
		expander.setToolTip(msg.clickToExpand());
		expander.setTemplate(tpl);
		configs.add(expander);

		ColumnConfig column = new ColumnConfig("approveStatus", "状态", 80);
		...
		column = new ColumnConfig("dateModified", "修改时间", 80);
		...
		ctBtn.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				BeanModel bean = BeanModelLookup.get().getFactory(
						Item.class).createModel(itemModel);
	...
	private void createQueryForm() {
		...
		tm = new Item();
		BeanModel bm = BeanModelLookup.get().getFactory(Item.class)
				.createModel(tm);
		queryFb.bind(bm);
