...
public class JpaDao<T, PK extends Serializable> extends SimpleJpaDao<T, PK>
	...
	public Page<T> findBy(final Page page, final String propertyName,
		...
		filters.add(new PropertyFilter("EQ_"+propertyName,value));
		...
		if (page.isAutoCount()) {
			int totalCount = countJpaQlResult(filter,filters);
			page.setTotalCount(totalCount);
		}
		query.setFirstResult(page.getStart());
		query.setMaxResults(page.getLimit());
	 ...
	 */
	public int countJpaQlResult(final String jpaQl,final List<PropertyFilter> filters) {

		String sql = " select count(e) from " + entityClass.getName() + " e ";
		if (jpaQl != null && jpaQl.trim().length() > 0)
			sql += " where " + jpaQl;
		String countQl = sql;
		...
		if (countQl.trim().length() > 0) {
			...
			query = this.buildPropertyFilterValues(query, filters);
	...
	@Override
	public int countJpaQlResult(String jpaQl) {
	...
	public List<T> find(List<PropertyFilter> filters) {
		...
		String sql = getBaseSql() +" where "+criterions ;
		Query query = entityManager.createQuery(sql);
		buildPropertyFilterValues(query,filters);
	...
	public Page<T> findPage(final Page<T> page,
		...
		String criterions = buildPropertyFilterCriterions(filters);
		String sql = getBaseSql();
		if (criterions != null && criterions.trim().length() > 0)
			sql += " where " + criterions;
		sql = dealOrder(sql,page);
		Query query = entityManager.createQuery(sql);
		if (page.isAutoCount()) {
			int totalCount = countJpaQlResult(criterions,filters);
			page.setTotalCount(totalCount);
		}
		//设置查询条件值
		query = buildPropertyFilterValues(query,filters);
		query.setFirstResult(page.getStart());
		query.setMaxResults(page.getLimit());
		List<T> lt = query.getResultList();
        page.setResult(lt);
	...
	/**
	 * 按属性条件列表给各个查询条件赋值,辅助函数.
	 */
	public Query buildPropertyFilterValues(Query query,final List<PropertyFilter> filters) {
		for (PropertyFilter filter : filters) {
			 Object obj = filter.getValue();
			 if (MatchType.LIKE.equals(filter.getMatchType())) {
				 obj = "%"+obj+"%";
			 }
			query.setParameter(filter.getPropertyName(),obj );
		}
	...
	public String buildPropertyFilterCriterion(String propertyName,
		...
		try {
		     if (MatchType.LIKE.equals(matchType)) {
				criterion = propertyName + " like :"+propertyName;
			}
            // 根据MatchType构造criterion
			if (MatchType.EQ.equals(matchType)) {
				criterion = propertyName + " = :"+propertyName;
			}
