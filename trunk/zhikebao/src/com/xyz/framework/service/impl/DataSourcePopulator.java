...
public class DataSourcePopulator implements IDataInit  {
    ...
    public void dataInit() throws Exception {
    	...
    	//全局权限，所有功能都必须先登录
    	res[0] = new Resource("只有登录用户才可访问系统",0, 0, "url","/**", 1, "全局资源");
    	res[22] = new Resource("首页链接",0, 0, "url","/mainservice", 1, "全局资源");
    	//销售模块
    	res[1] = new Resource("销售",100, 1, "menu","/salemain", 1, "销售管理");
    	res[2] = new Resource("新建订单",101, 100, "menu","/salenew", 1, "销售管理");
    	res[3] = new Resource("确认订单",102, 100, "menu","/salecfm", 2, "销售管理");
    	res[4] = new Resource("收款发货",103, 100, "menu","/salesnd", 3, "销售管理");
        res[5] = new Resource("订单查询",104, 100, "menu","/saleqry", 4, "销售管理");
        res[6] = new Resource("售后服务",105, 100, "menu","/salesrv", 5, "销售管理");
        //库存管理
        res[7] = new Resource("库存",500, 1, "menu","/stockmain", 5, "库存管理");
    	res[8] = new Resource("商品入库",501, 500, "menu","/stockin", 1, "库存管理");
    	res[9] = new Resource("商品出库",502, 500, "menu","/stockout", 2, "库存管理");
    	res[10] = new Resource("库存查询",503,500, "menu","/stockqry", 3, "库存管理");
        res[11] = new Resource("库存状态",504, 500, "menu","/stocksta", 4, "库存管理");
        //客户管理
        res[12] = new Resource("客户",300, 1, "menu","/custmain", 3, "客户管理");
    	res[13] = new Resource("客户管理",301, 300, "menu","/custmng", 1, "客户管理");
    	res[14] = new Resource("客户分析",302, 300, "menu","/custalz", 2, "客户管理");
    	res[15] = new Resource("客户报价",303, 300, "menu","/custopp", 3, "客户管理");
        res[16] = new Resource("客户关怀",304, 300, "menu","/custcon", 4, "客户管理");
        
        //采购管理
        res[17] = new Resource("采购",400, 1, "menu","/buymain", 4, "采购管理");
    	res[18] = new Resource("新建采购",401, 400, "menu","/buynew", 1, "采购管理");
    	res[19] = new Resource("收货入库",402, 400, "menu","/buyin", 2, "采购管理");
    	res[20] = new Resource("商品目录",403, 400, "menu","", 3, "采购管理");
        res[21] = new Resource("商品列表",404, 400, "menu","/pdtlist", 4, "采购管理");
        
        //系统管理
        res[23] = new Resource("管理",200, 1, "menu","/buymain", 2, "系统管理");
    	res[24] = new Resource("员工维护",201, 200, "menu","/buynew", 1, "员工维护");
    	res[25] = new Resource("用户管理",202, 200, "menu","/buyin", 2, "用户管理");
    	res[26] = new Resource("权限维护",203, 200, "menu","", 3, "权限维护");
    	res[27] = new Resource("功能维护",204, 200, "menu","", 4, "权限维护");
        res[28] = new Resource("代码维护",205, 200, "menu","/pdtlist", 5, "代码维护");
    	...
    	List<Resource> sks = new ArrayList();
    	//A_USER
    	as[0] = new Authority("A_USER","用户");
    	sks.add(res[0]);
    	sks.add(res[22]);
    	as[0].setResources(sks);
        //A_SYS_ADMIN
    	as[3] = new Authority("A_SYS_ADMIN","系统管理员");
    	List<Resource> syssks = new ArrayList();
    	syssks.add(res[23]);
    	syssks.add(res[24]);
    	syssks.add(res[25]);
    	syssks.add(res[26]);
    	syssks.add(res[27]);
    	syssks.add(res[28]);
    	as[3].setResources(syssks);
    	//A_ADMIN
    	as[1] = new Authority("A_ADMIN","管理员");
    	List<Resource> asks = new ArrayList();
    	asks.add(res[23]);
    	asks.add(res[24]);
    	asks.add(res[25]);
    	asks.add(res[26]);
    	asks.add(res[27]);
    	asks.add(res[28]);
    	as[1].setResources(asks);
    	
    	//A_SALE
    	as[2] = new Authority("A_SALE","销售经理");
    	List<Resource> srs = new ArrayList();
    	for(int i=1;i<7;i++)
    	{
    		srs.add(res[i]);
    	}
    	as[2].setResources(srs);
    	for(Authority a : as)
    		ss.saveAuth(a);
    	...
    	*/
    	...
    	User admin = new User("admin","root","",true,"文浪","valiant",true,true,true);
    	List<Authority> urs = new ArrayList();
    	urs.add(as[1]);
    	urs.add(as[0]);
    	urs.add(as[2]);
    	urs.add(as[3]);
    	admin.setZkbAuthorities(urs);
    	ss.saveUser(admin);
