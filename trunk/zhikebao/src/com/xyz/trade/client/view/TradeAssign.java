...
public class TradeAssign extends LayoutContainer {
	...
	private void createExpander() {
		...
		RpcProxy<PagingLoadResult<TradeModel>> proxy = new RpcProxy<PagingLoadResult<TradeModel>>() {
			...
			protected void load(Object loadConfig,
				...
				tm.setStatus("WAIT_SELLER_SEND_GOODS");
				tm.setOwnerId(0);
	...
	public Dialog createSelDainlog()
	{
        ...
        final Grid<BeanModel> usergrid = createGrid();
		       ...
		       public void componentSelected(ButtonEvent ce) {  
		    	   ...
		    	   if(um!=null)
		    	   {
		    		   ...
		    		   if(lb==null||lb.size()<1)
					   {
							...
							tradeKeys = new Integer[lb.size()];
							for(int i=0;i<lb.size();i++)
							{
								BeanModel bm = lb.get(i);
								tradeKeys[i] = ((TradeModel)bm.getBean()).getPid();
							}
						}
		    		   service.assignOwner(tradeKeys, user.getPid(), new AsyncCallback<Boolean>() {
